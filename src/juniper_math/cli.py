...
try:
    from juniper_math import full_pipeline, pilot_pipeline, sft_pipeline, train_pipeline
...
def _cmd_train_full_run(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        report = full_pipeline.run_full_train(
            config_path=config_path,
            max_steps=args.max_steps,
            eval_sample_size=args.eval_sample_size,
            milestone_eval=not args.no_milestone_eval,
        ...
        print(f"FAIL: full training aborted: {exc}", file=sys.stderr)
        return 1
    print(f"PASS: full training run {report.training_config.run_id} complete")
    print(f"  device:                 {report.device}")
    print(f"  parameters:             {report.parameter_count:,}")
    print(f"  train packed sequences: {report.train_packed_sequences:,}")
    print(f"  train padding fraction: {report.train_padding_fraction:.4f}")
...
def _cmd_train_full_resume_test(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        report = full_pipeline.run_full_resume_test(config_path=config_path)
    except JuniperConfigError as exc:
        print(f"FAIL: full resume comparison aborted: {exc}", file=sys.stderr)
        return 1
    for key, value in report.as_dict().items():
        print(f"{key}: {value}")
    print(
...
def _cmd_full_evaluate(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        reports = full_pipeline.evaluate_full_checkpoint(
            Path(args.checkpoint), config_path=config_path, sample_size=args.sample_size
        )
    except (JuniperConfigError, FileNotFoundError) as exc:
        print(f"FAIL: full evaluation aborted: {exc}", file=sys.stderr)
        return 1
    for suite_name, summary in reports.items():
        print(f"\n[{suite_name}]")
        for key, value in summary.items():
...
def _cmd_full_infer(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        result = full_pipeline.run_full_infer(
            Path(args.checkpoint), args.prompt, args.max_new_tokens, config_path=config_path
        )
    except (JuniperConfigError, CheckpointError, FileNotFoundError) as exc:
        print(f"FAIL: full inference aborted: {exc}", file=sys.stderr)
        return 1
    print(result.text)
    return 0


def _cmd_train_sft_run(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        report = sft_pipeline.run_sft_train(
            config_path=config_path,
            max_steps=args.max_steps,
            eval_sample_size=args.eval_sample_size,
            milestone_eval=not args.no_milestone_eval,
        ...
        print(f"FAIL: SFT training aborted: {exc}", file=sys.stderr)
        return 1
...
def _cmd_train_sft_resume_test(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        report = sft_pipeline.run_sft_resume_test(config_path=config_path)
    except JuniperConfigError as exc:
        print(f"FAIL: SFT resume comparison aborted: {exc}", file=sys.stderr)
        return 1
    for key, value in report.as_dict().items():
        print(f"{key}: {value}")
    print(
...
def _cmd_sft_evaluate(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        reports = sft_pipeline.evaluate_sft_checkpoint(
            Path(args.checkpoint), config_path=config_path, sample_size=args.sample_size
        )
    except (JuniperConfigError, FileNotFoundError) as exc:
        print(f"FAIL: SFT evaluation aborted: {exc}", file=sys.stderr)
        return 1
    for suite_name, summary in reports.items():
        print(f"\n[{suite_name}]")
        if isinstance(summary, dict):
            for key, value in summary.items():
                if key == "category_accuracy" and isinstance(value, dict):
                    print("  category_accuracy:")
                    for cat, acc in value.items():
                        print(f"    {cat}: {acc:.4f}")
                else:
...
def _cmd_sft_infer(args: argparse.Namespace) -> int:
    if (fail := _require_train_module()) is not None:
        return fail
    config_path = Path(args.config) if args.config else None
    try:
        result = sft_pipeline.run_sft_infer(
            Path(args.checkpoint), args.prompt, args.max_new_tokens, config_path=config_path
        )
    except (JuniperConfigError, CheckpointError, FileNotFoundError) as exc:
        print(f"FAIL: SFT inference aborted: {exc}", file=sys.stderr)
        return 1
    print(result.text)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    full_run_parser = train_sub.add_parser(
        "full-run", help="Run Phase 7 full base pretraining from config/training_phase7_full.yaml"
    )
    full_run_parser.add_argument(
        "--config",
        default=None,
        help="full training config path (default: config/training_phase7_full.yaml)",
    )
    full_run_parser.add_argument(
        "--max-steps", type=int, default=None, help="override schedule.total_steps (for bounded checks)"
    )
    full_run_parser.add_argument(
        "--eval-sample-size",
        type=int,
    ...
    full_run_parser.add_argument(
        "--resume-from", default=None, help="checkpoint path to resume an interrupted canonical run from"
    )
    full_run_parser.set_defaults(func=_cmd_train_full_run)

    full_resume_parser = train_sub.add_parser(
        "full-resume-test",
        help="Sec. 24-style gate at Phase 7 scale: compare an uninterrupted run against an "
        "interrupted, resumed run",
    )
    full_resume_parser.add_argument("--config", default=None, help="full training config path")
    full_resume_parser.set_defaults(func=_cmd_train_full_resume_test)

    sft_run_parser = train_sub.add_parser(
        "sft-run", help="Run Phase 8 SFT from config/training_phase8_sft.yaml, starting from the Phase 7 Base"
    )
    sft_run_parser.add_argument(
        "--config", default=None, help="SFT training config path (default: config/training_phase8_sft.yaml)"
    )
    sft_run_parser.add_argument(
        "--max-steps", type=int, default=None, help="override schedule.total_steps (for bounded checks)"
    )
    sft_run_parser.add_argument(
        ...
        type=int,
        default=None,
        help="cases per suite at each milestone (default: the complete suite)",
    )
    sft_run_parser.add_argument(
        "--no-milestone-eval",
        action="store_true",
        help="skip the milestone capability/tool-interaction evaluation",
    )
    sft_run_parser.add_argument(
        "--resume-from", default=None, help="checkpoint path to resume an interrupted SFT run from"
    )
    sft_run_parser.set_defaults(func=_cmd_train_sft_run)

    sft_resume_parser = train_sub.add_parser(
        "sft-resume-test",
        help="Sec. 18 gate for the Phase 8 SFT pipeline: compare an uninterrupted run against an "
        "interrupted, resumed run",
    )
    sft_resume_parser.add_argument("--config", default=None, help="SFT training config path")
    sft_resume_parser.set_defaults(func=_cmd_train_sft_resume_test)
    ...
    full_evaluate_parser = subparsers.add_parser(
        "full-evaluate",
    ...
    full_evaluate_parser.set_defaults(func=_cmd_full_evaluate)

    full_infer_parser = subparsers.add_parser(
        "full-infer", help="Generate text from a Phase 7 checkpoint for a single prompt"
    )
    full_infer_parser.add_argument("--checkpoint", required=True, help="path to a training checkpoint (.pt)")
    full_infer_parser.add_argument("--prompt", required=True)
    full_infer_parser.add_argument("--max-new-tokens", type=int, default=32)
    full_infer_parser.add_argument("--config", default=None, help="full training config path")
    full_infer_parser.set_defaults(func=_cmd_full_infer)

    sft_evaluate_parser = subparsers.add_parser(
        "sft-evaluate",
        help="Phase 8 evaluation: the four frozen v2 suites plus the Phase 8 tool-interaction suite",
    )
    sft_evaluate_parser.add_argument(
        "--checkpoint", required=True, help="path to a training checkpoint (.pt)"
    )
    sft_evaluate_parser.add_argument("--config", default=None, help="SFT training config path")
    sft_evaluate_parser.add_argument(
        "--sample-size",
        type=int,
        default=None,
        help="number of cases per suite to evaluate (default: each complete suite)",
    )
    sft_evaluate_parser.set_defaults(func=_cmd_sft_evaluate)

    sft_infer_parser = subparsers.add_parser(
        "sft-infer", help="Generate text from a Phase 8 SFT checkpoint for a single prompt"
    )
    sft_infer_parser.add_argument("--checkpoint", required=True, help="path to a training checkpoint (.pt)")
    sft_infer_parser.add_argument("--prompt", required=True)
    sft_infer_parser.add_argument("--max-new-tokens", type=int, default=48)
    sft_infer_parser.add_argument("--config", default=None, help="SFT training config path")
    sft_infer_parser.set_defaults(func=_cmd_sft_infer)
