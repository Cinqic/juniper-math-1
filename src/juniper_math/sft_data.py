...
from juniper_math.sft_curriculum import build_independent_direct_examples, build_independent_safety_examples
...
SFT_DATASET_ID = "juniper-math-sft-v6"
SFT_MANIFEST_SCHEMA_VERSION = "6.0.0"
SFT_RENDERING_SCHEMA_VERSION = "6.0.0"
...
def select_and_record_sft_subset(
    ...
    if independent_safety_examples_per_category:
        for split in selections:
            independent = build_independent_safety_examples(
                split, independent_safety_examples_per_category, seed
            )
            for ex in independent:
                tokenize_and_mask(ex, tokenizer, max_sequence_length)
            selections[split].extend(independent)
